...
impl Request {
    pub async fn new(stream: &mut TcpStream) -> std::io::Result<Self> {
        ...
        let request_text = String::from_utf8(received).map_err(|err|std::io::Error::other(err))?;
        let request_lines: Vec<&str> = request_text.split_inclusive('\n').collect();
        ...
        let http_method = parts.next().ok_or(std::io::Error::other("failed to unwrap method"))?;
        let full_path = parts.next().ok_or(std::io::Error::other("failed to unwrap path"))?;
